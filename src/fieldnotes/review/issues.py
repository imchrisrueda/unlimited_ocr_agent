...
def extract_review_issues_from_document(doc: EstadilloDocument) -> list[ReviewIssue]:
    """Convierte incertidumbres y ExtractionWarnings de EstadilloDocument en ReviewIssues deterministas.
...
def extract_invalid_region_issues(page_dtos: list[EstadilloPageDTO]) -> list[ReviewIssue]:
    ...
    for dto in page_dtos:
        ...
        # 3. Regiones de filas
        current_col = None
        for idx, row_dto in enumerate(dto.rows):
